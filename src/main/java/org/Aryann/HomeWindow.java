...
public class HomeWindow extends JFrame {
    private int userId;  // declare userId as a field
    ...
    public HomeWindow(int userId) throws SQLException {
        this.userId = userId;  // assign the provided userId to the class field
    ...
    selectionPanel.add(Box.createRigidArea(new Dimension(30, 0)));
    selectionPanel.add(userLabel);
        
        JButton newButton = new JButton("Add Expenses");
        newButton.addActionListener(new MyActionListener(userId));

        JComboBox<String> categoriesBox;
        categoriesBox = createSortPanel();

        JButton calculateButton = new JButton("Summary");
        calculateButton.addActionListener(new MyActionListener(userId));
